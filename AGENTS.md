...
- Each bot **has a computer** — `@cloudflare/computer` `Workspace` on `BotActor` (DO SQLite) with **Worker shell** for bash. Sell the computer. Do not add a `computers` table, shared desk, takeover pane, `computer.sleep`, or a Computer Durable Object. The office pane is this bot’s screen.
