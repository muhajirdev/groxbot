...
- **Cloudflare first. Actor model first.** One Durable Object per `botId`: `BotActor` extends Agents `Agent`. Instance name is `botId`, never `threadId` / `roomId`. v1 is one home-office thread. Office chat is Pi `runAgentLoopContinue` streamed over Cap’n Web (`/bots/:botId/rpc`); the log lives in DO SQLite (`office_chat`). A later group room is a **separate** Durable Object (`RoomActor`, name = `roomId`) that does **not** run the model — coordinator only (log, members, floor, websockets). Do not add `SessionManager` yet. Do not use a custom `SessionProvider` or Cloudflare Session as the office catalog. One Durable Object per live app (`AppRuntime`, name = `appId`). See [docs/rooms-plan.md](./docs/rooms-plan.md).
- Each bot **has a computer** — `@cloudflare/computer` `Workspace` on `BotActor` (DO SQLite) with **Worker shell** (`just-bash`) for bash. Sell the computer. Do not add a `computers` table, shared desk, takeover pane, `computer.sleep`, a Computer Durable Object, or `@cloudflare/shell` / `state.*` as a second filesystem. The office pane is this bot’s screen.
...
- Shared team data lives in Postgres, not in the DO. Office chat is Pi on `BotActor` (DO SQLite log + Cap’n Web). Postgres threads stay for poke / guest continue — not the office transcript.
- Hosted brain: Pi `runAgentLoopContinue` on `BotActor` for v1 office (computer + soul + tools). Owned-message turns (poke / guest / REST gateway) use the same loop. Tests use `ScriptedAgentRuntime`. Do not treat Flue as the product loop.
