...
- Workspace apps (docs / slides / sheets) belong to the workspace, not a computer or thread. Factory templates stamp `client.js` / `server.js` into the App Durable Object; that copy plus document state are the source of truth. Docs/slides/sheets UI is derived from Cloudflare OS gadgets (Apache 2.0); the iframe host implements their RPC against `gadget.load`/`save`. Do not expose computer, thread, gadget, blueprint, or source in the product UI. Do not add Cap’n Web or Yjs for apps.
