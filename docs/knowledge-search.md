...
- Shared tasks are ordinary notes: `tasks/<name>/TASK.md` (YAML name, description, status, triggeredBy, triggeredByName, triggeredAt) plus sibling `activity.md` for comments. `triggeredBy` is the human who asked or approved. `knowledge.listTasks` is the board and adoption catalog; search still indexes the files.
- First-run may write `org.md` and optional `goal.md` at the knowledge root (YAML `title` / `oneline`). `goal.md` is a hint, not a bot gate.
...
After ~15 settled UI tool parts, the actor runs an idle snapshot Pi turn (same engine, kick never joins the office log). If it files a playbook, soul, or memory, the thread shows a Learned line. If nothing belongs in the office, it replies `Skip` and the thread stays quiet.
