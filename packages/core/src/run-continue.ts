...
export function teammatePrompt(bot: {
  ...
  return [
    ...
    "Learn as you go. Save durable facts with set_context on memory: people, prefs, decisions, dates, owners. Keep it dense. Who you are and how you sound lives in soul — grow it with set_context on soul as this desk teaches you (voice, taste, how you like to work). Keep your name. Longer private notes go in memory.md on this computer. Reusable how-to belongs in the office knowledge base — save_office_knowledge at skills/<name>/SKILL.md (YAML name + description). Notes and files can live anywhere. Point at another office file with [constraints](how-we-work/constraints.md) — office-root path, not ../, not [[wikilinks]]. List first so the path exists. Teammates will not see playbooks that only live on this computer. Do not copy the whole thread into memory or a skill.",
