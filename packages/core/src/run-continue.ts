...
import { assertWorkspacePlanAllowed } from "./billing.js";
...
export const KNOWLEDGE_EXECUTE_HINT =
  "`knowledge` — shared office library, not this computer. `await knowledge.search({ query })` then `await knowledge.read({ path })` / `write({ path, content })` or `from` for a computer PDF. search and read also accept a lone string. Skills (playbooks) use `skill_manage`, not `knowledge.write`. Shared tasks are `tasks/<name>/TASK.md` (YAML name, description, status, triggeredBy, triggeredByName, triggeredAt). triggeredBy is the human who asked or approved — never you. Comments append to sibling `activity.md` as `## ISO-8601 Name @userId` — not a room, not this computer. " +
...
function hintExecuteConnector(
  ...
  if (description.includes(`await ${name}.`)) return description;
  const hinted = `- ${hint}`;
  const bare = new RegExp(`(?:^|\\n)- \`${name}\`$`, "m");
  if (bare.test(description)) {
    return description.replace(bare, (row) =>
      row.startsWith("\n") ? `\n${hinted}` : hinted,
    );
  }
...
export async function startOfficeRun(opts: {
  ...
  try {
    ...
    const message =
      error instanceof Error ? error.message : WORKSPACE_PLAN_REQUIRED_MESSAGE;
