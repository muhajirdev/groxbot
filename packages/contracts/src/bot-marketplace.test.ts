...
describe("BOT_MARKETPLACE_CATALOG", () => {
  it("has stable unique ids and required fields", () => {
  ...
  it("derives starter jobs that include Chief of Staff and skip empty names", () => {
    ...
    expect(BOT_MARKETPLACE_STARTER_JOBS.every((name) => name.trim().length > 0)).toBe(
      true,
    );
...
describe("filterBotMarketplace", () => {
  it("matches name and blurb case-insensitively", () => {
